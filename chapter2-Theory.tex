...
\section{The QCD Lagrangian}
	...
	We obtain the QCD Lagrangian by considering the spin-$\frac{1}{2}$ Dirac Lagrangian for the case of $f$
	fermionic fields each with mass $m_f$:

	\begin{equation}
		\mathcal{L}_D = \sum_{f=1}^{6}\left(i\overline{\psi}_i^{(f)}\slashed{\partial}_{ij}\psi_j^{(f)} - m_f\overline{\psi}^{(f)}_i\psi_i^{(f)}\right),
		\label{eqn:QCDLagrangian}
	\end{equation}

	\noindent where $\psi_i$ is vector of 3 fermion fields in the fundamental representation of $SU(3)$.
	This is manifestly invariant under the \emph{global} $SU(3)$ transformation
	...
	We can however promote this symmetry to a \emph{local} one by relaxing the constraint that $\alpha^a$ are
	constant and instead allow them to depend on a space-time coordinate $\alpha^a = \alpha^a(x^\mu)$.  This
	breaks the $SU(3)$ symmetry but we can recover the required transformation properties by replacing the
	usual derivative term with a `covariant derivative' defined by:
	...
	With this replacement the local $SU(3)$ invariance of \eqref{eqn:QCDLagrangian} is recovered.  We can also consider
	the effect of self-interaction terms of the gluon field on our theory.  The field-strength tensor for $A^a_\mu$,
	$F^a_{\mu\nu}$ is given by:
	...
	\begin{itemize}
		\item Talk about extending this to the quantum Lagrangian,
		\item Talk about renormalisation
		\item Talk a bit about ghosts and Fadeev-Popov (already written),
	\end{itemize}

\section{QCD Feynman Rules}

	\begin{itemize}
		\item From the Lagrangian to the Feynman rules
	\end{itemize}
	...
	Where $DA=\prod_x\prod_{a, \mu}dA^a_\mu$.  As briefly outlined above we would like to perform a functional integration over all possible gauge choices and then pick out the subset of gauges we are interested in by enforcing the gauge condition $G(A)=0$ to eliminate over-counting.  This constraint may be written as ~\cite{p&s}:
	...
	but because the QCD gauge transformation is more involved than the QED equivalent the determinant term still depends on $A_\mu$:
	...
	These non-physical modes are called the Fadeev-Popov ghosts/anti-ghosts and are a consequence of enforcing gauge invariance - they are represented by the final term in equation (12a).

	\subsection{Renormalising the QCD Lagrangian}

	Similarly to QED we anticipate divergent quantities in our calculations above tree level and so we introduce counter terms into our Lagrangian.  Similarly to equation (4) we have the following relations:
	...
	Inserting these into equations (13) and (14) and rearranging so that we have $\mathcal{L^{QCD}} = \mathcal{L}_0 + \mathcal{L}_{ct}$ where $\mathcal{L}_0$ is as defined in equation (13) and the counter-term Lagrangian is:
