...
\section{The QCD Lagrangian}
	...
	as those where no scattering occurs or those with bubble contributions).\\
	The full set of rules for the vertices and propagators are summarised in table (\ref{tab:feynRules}).  The remaining `Feynman rules' may be summarised as:
	...
	we go from the matrix element of some scattering process to a useful physical observable: The ``cross-section'', $\sigma$.  The matrix element
	is related to the fully-differential cross-section by `Fermi's golden rule' which, for a scattering process
	$p_{a} + p_{b}\rightarrow p_{1} + \ldots + p_{m}$ is given by

	\begin{align}
	\begin{split}
		d\sigma = &\frac{|\mathcal{M}(p_{a} + p_{b}\rightarrow p_{1}^{(f)}, \ldots, p_{m}^{(f)})|^2}{F}\\
		    \times&(2\pi)^2\delta^{(4)}(p_{a} + p_{b} - p_{1} - \ldots - p_{m}) \\
		    \times&\frac{d^3\vec{p}_1}{2E_1(2\pi)^2}\cdots\frac{d^3\vec{p}_m}{2E_m(2\pi)^2},
	\end{split}
	\end{align}

	where $F=4\sqrt{(p_ap_b)^2 - m_a^2m_b^2}$ is the flux of the incoming particles given by
