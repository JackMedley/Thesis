...
\section{The QCD Lagrangian}
	...
	This is the LSZ reduction formula for $2\rightarrow2$ scattering in $\phi^3$-theory.  It reduces the problem of finding scattering
	...
	as free.  The next step is to see how we can calculate these time-ordered products. We can most conveniently do this by taking
	...
	where $J$, $\eta$, $\overline{\eta}$, $\chi$ and $\overline{\chi}$ are `source' terms.  We can rewrite this as a
	combination of functional derivatives acting on the free QCD path integral, $\mathcal{Z}_0$ as:
	...
	$\mathcal{L}_{\text{QCD}, 0}$ in place of the full interacting theory $\mathcal{L}_{\text{QCD}}$.
	We can solve $\mathcal{Z}_0$ exactly which gives us the propagators for the gluons, quarks and ghosts.
	Respectively:
	...
	\begin{subequations}
		...
		\end{equation}
		\begin{equation}
			\langle0|c_a(x)c_b(y)|0\rangle = \int \frac{d^4k}{(2\pi)^4}
				e^{-ik\cdot(x-y)}\delta_{ab}\frac{i}{k^2}.
		\end{equation}
	\end{subequations}
