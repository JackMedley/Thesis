...
\section{QCD Factorisation at Hadronic Colliders}
	...
	So far we have only talked about the idea of two particles interacting and scattering off one another in to
	some final state which we are interested in.  This is too simple a picture when we are considering hadronic
	colliders such as the Large Hadron Collider (proton-proton), the Tevatron (proton-antiproton), HERA (proton-lepton) and, potentially,
	a Future Circular Collider (FCC).\\At experiments we collide QCD bound states with one another but in practise when calculating
	cross-sections we perform a sum over the possible combinations of initial states we may encounter in the two incoming hadrons.
	In order to do this we must have some understanding of the dynamics of partons inside the onrushing hadrons; this is where
	Parton Distribution Functions (PDFs) come in.  A PDF, $f_i(x, Q^2)$ is a probabilistic functions which tells us how
	likely we are to find a parton of type $i$ carrying a fraction $x$ of the total hadrons momentum in a hadron during a
	collision with an energy scale $Q$.

\section{From Partons to Jets}

	{\color{red}
	\begin{itemize}
		\item QCD confinement (briefly),
		\item Hadronisation,
		\item How to model jets from partons ($k=0, \pm1$),
		\item Anti-$k_T$ and the parameters involved
	\end{itemize}
	}
