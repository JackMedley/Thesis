...
\section{High Energy Jets}
	...
	\subsection{Matching to \ARIADNE}
		...
		and such emissions also lead to large logarithmic enhancements similar to those discussed in this
		...
		dominant force.  In this way fixed order predictions can be augmented to also contain the
		...
		a hard scatter: it considers the splitting of colour dipoles into more colour dipoles rather than
	...
	\subsection{Comparisons to data}
		...
		other state-of-the-art theoretical descriptions.  In some cases we do well even when we are far
		from this strict limit; in areas with soft radiation or limited rapidity regions in which to radiate.
		We discuss a few specific examples below.
		...
		Fig.~\eqref{fig:wJetsEg} describes the differential distribution fir $W^\pm$ plus dijets in terms of the
		invariant mass of the dijet system defined by the two leading jets in $p_\perp$, $m_{12}$.  Large values
		of $m_{12}$ correspond to the High Energy limit and so we expect that \hej should describe well this region.
		In fact, \HEJ gives the best description of data across almost the entire range of $m_{jj}$ with the other
		theoretical predictions only agreeing with data below an invariant mass of approximately $300$~GeV and deviate
		badly by $2$~TeV.  This high invariant mass region is an important region for VBF studies and so it is
		important that we are able to describe QCD emissions.  The failure of fixed order calculations show that
		perturbative stability already begins to breakdown in $7$~TeV LHC data.  As we will see in
		chapter~\ref{chap:100TeV} this is a challenge we must face at a potential high energy Future Circular Collider.
