...
\section{$\zg$+Jets at the LHC}
	...
	\subsection{$\zg$+Jets at the CMS Experiment}
		...
		\subsubsection{Jet Multiplicity and Differential Distributions}
		...
		\subsubsection{A Double Differential Study of the Drell-Yan Cross-section}

		Throughout the course of this work many analyses were used to compare \hej to experimental
		data.  Though we include only those studies illuminating for a discussion of higher order
		logarithmic corrections of QCD processes at hadronic colliders we here discuss a comparison
		of \HEJ to data collected at the CMS experiment~\cite{CMS:2014vtk}.  In particular, we consider
		...
		The study focussed on 0-, 1- and 2-jet events in addition to a di-muon pair though since we can
		...
		The figures for this section have been discussed previously in section~\ref{sec:MC}.
		...
		importance sampling in \HEJ.  As mentioned during the discussion of importance
		sampling techniques in Monte Carlo it was necessary to modify the Breit-Wigner
		sampling to a flat generation of the di-muon mass in order to generate predictions
		for the invariant mass bins far from dominant region.
...
\section{$\zg$+Jets Conclusions}
	...
	section~\ref{sec:HEQCD} by motivating and describing the construction of an
	...
	described in section~\ref{sec:Constructing} where we showed explicitly the
	...
	fixed-order-based predictions do not fare as well.  Discrepancies which occur
	...
	with dijets. The impact of the high-energy
